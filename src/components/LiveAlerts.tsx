            ...
            <div className={`flex-shrink-0 flex gap-1 ${maximized ? 'px-6 py-4 border-b border-[#2A2A28] bg-[#111111]' : 'px-3 py-2 border-b border-[rgba(255,255,255,0.05)]'}`}>
              {(['all', 'news', 'quakes', 'feeds'] as const).map(f => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`px-3 py-1.5 rounded text-[10px] font-mono tracking-wider transition-all ${filter === f ? 'bg-[var(--cyan-primary)]/20 text-[var(--cyan-primary)] border border-[var(--cyan-primary)]/50' : 'text-[#8A8880] border border-transparent hover:text-[#DEE4E7] hover:bg-[#2A2A28]'}`}
                >
                  {f.toUpperCase()}
                </button>
              ))}
                    ...
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start gap-1.5 mb-2">
                          ...
                          <span className={`text-[10px] font-mono text-[#DEE4E7] leading-relaxed ${alert.type === 'news' ? 'line-clamp-3' : 'truncate'}`}>
                        ...
                        <div className="flex items-center justify-between border-t border-[#2A2A28]/50 pt-1.5 mt-1.5">
                          <div className="flex items-center gap-2">
                            ...
                            {alert.time && (
                              <span className="text-[9px] font-mono text-[#5A6B7A] flex items-center gap-1 border-l border-[#2A2A28] pl-2">
